...
// Props del renderer: el bucle se activa con isEnabled; songToken cambia al cambiar
// de pista para que el bridge reinicie el LUFS integrado.
type RendererProps = { isEnabled: boolean; songToken: number };
...
// ---- preset "Analisis Master" (valores leídos del .mms) ----
const PRESET = {
	// Module.* size -> proporción de anchura; order -> posición
	rowWide: [516, 311, 150, 594, 189, 200], // Spectrogram, Waveform, Oscilloscope, Spectrum, Stereometer, Loudness
	rowsStacked: [
		[516, 311, 150],
		[594, 189, 200]
	],
	...
	// escala expandida arriba: los primeros 24 LU ocupan el 72% de la barra, porque
	// la música masterizada vive entre 0 y -20 LUFS y en escala lineal 0..-50 no se mueve.
	loudness: { scaleLu: 50, expandLu: 24, expandFrac: 0.72, ticks: [0, 6, 12, 24, 36, 50] }
};

// ---- tema Darktable (Themes/Darktable.ini) ----
const T = {
	bg: "#000000",
	bg2: "rgb(48,48,48)",
	...
	error: "rgba(252,8,3,0.75)",
	tooltipBg: "rgba(12,12,12,0.92)",
	tooltipText: "rgb(192,192,192)",
	wave: "rgba(192,192,192,0.31)",
	hLow: "rgba(48,136,208,0.75)",
	hMid: "rgba(252,8,3,0.75)",
	hHigh: "rgba(255,192,71,0.75)",
	stGuides: "rgb(92,92,92)",
	stLow: "rgba(48,136,208,0.75)",
	stMid: "rgba(252,8,3,0.75)",
	stHigh: "rgba(255,192,71,0.75)",
	stPrimary: "rgb(255,255,255)",
	loudPrimary: "rgb(164,164,164)",
	loudText: "rgb(0,0,0)",
	osc: "rgb(164,164,164)",
	specLine: "rgba(164,164,164,0.63)",
	specSecondary: "rgba(127,127,127,0.75)",
	specFreqLines: "rgb(96,96,96)"
};
// colormaps/Darktable.map
...
const binFreq = (i: number) => melToHz(MEL_LO + ((i + 0.5) / NMEL) * (MEL_HI - MEL_LO));
const xOfFreq = (f: number, w: number) => ((hzToMel(f) - MEL_LO) / (MEL_HI - MEL_LO)) * w;
const freqOfX = (x: number, w: number) => melToHz(MEL_LO + (x / w) * (MEL_HI - MEL_LO));
const tiltAt = (f: number, dbPerOct: number) => dbPerOct * Math.log2(f / 1000);

const NOTE_NAMES = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
function noteOf(f: number): { name: string; cents: number } {
...
// ---- estado persistente del renderer ----
type Area = { x: number; y: number; w: number; h: number };
type WaveCol = { min: number; max: number; low: number; mid: number; high: number };
type State = {
	isError: boolean;
	link: MeterLink | null;
	...
	tilt: Float32Array; // por bin Mel, dB
	specLine: Float32Array;
	specFillT: Float32Array;
	specFill: Float32Array;
	spectro: { canvas: HTMLCanvasElement | null; w: number; h: number; rowBin: Int32Array; col: ImageData | null };
	wave: { cols: WaveCol[]; head: number };
	bandSm: { low: number; mid: number; high: number };
	osc: { gain: number };
	stereo: { gain: number };
	loud: { holdL: number; holdR: number };
	mouse: { x: number; y: number } | null;
	onMove: ((e: MouseEvent) => void) | null;
	onLeave: (() => void) | null;
	silentSince: number; // fade_modules_during_silence
	alpha: number;
	lastT: number; // ts del último frame ingerido (decaimientos indep. de fps)
	nextRenderT: number; // cap del render a 144 fps
	spectroSeq: number; // último seq de datos dibujado en el espectrograma
};
const WAVE_CAP = 4096;
...
// ---- ingestión de un frame nuevo (una vez por frame del bridge) ----
function ingest(S: State, f: Float32Array, now: number) {
	...
	// suavizado en frecuencia para el relleno de color (±5 bins)
	const K = 5;
	let acc = 0;
	...
	c.min = f[H.WAVE_MIN];
	c.max = f[H.WAVE_MAX];
	// las bandas se guardan suavizadas: sin esto el historial se lee como ruido y tapa la onda
	const B = S.bandSm;
	...
	c.high = B.high;
	S.wave.head++;
	// true peak hold: 12 dB/s, independiente de fps (dt-based)
	S.loud.holdL = Math.max(f[H.TP_L], S.loud.holdL - 12 * dt);
	S.loud.holdR = Math.max(f[H.TP_R], S.loud.holdR - 12 * dt);
}

// ---- módulos ----
function drawSpectrogram(ctx: CanvasRenderingContext2D, a: Area, S: State, f: Float32Array | null, nNew: number) {
	const sp = S.spectro;
	...
	if (octx && f && nNew > 0 && sp.col) {
		const adv = Math.min(nNew, 24); // px a avanzar = frames de datos nuevos (cap tras stalls)
...
function drawWaveform(ctx: CanvasRenderingContext2D, a: Area, S: State, s: (px: number) => number) {
	const colW = Math.max(1, Math.round(s(PRESET.waveform.colPx)));
	const n = Math.min(WAVE_CAP, Math.floor(a.w / colW));
	const yc = a.y + a.h / 2;
	const half = a.h / 2;
	const gain = 1.4; // ~-3 dBFS llena la altura; deja aire para ver el historial de bandas
	const floor = PRESET.waveform.floorDb;
	const yDb = (db: number) => a.y + a.h * clamp(1 - (db - floor) / -floor, 0, 1);
	ctx.save();
	ctx.beginPath();
	ctx.rect(a.x, a.y, a.w, a.h);
	ctx.clip();
	// timecode (1 s = 50 columnas)
	...
	ctx.stroke();
	// historial de volumen por banda
	const bands: ["low" | "mid" | "high", string][] = [
		["low", T.hLow],
		["mid", T.hMid],
		["high", T.hHigh]
	];
...
function drawOscilloscope(ctx: CanvasRenderingContext2D, a: Area, S: State, f: Float32Array | null, s: (px: number) => number) {
	if (!f) return;
	let peak = 1e-4;
...
function drawSpectrum(ctx: CanvasRenderingContext2D, a: Area, S: State, f: Float32Array | null, s: (px: number) => number) {
	const { centerDb, rangeDb, targetOffset } = PRESET.spectrum;
	const bottomDb = centerDb - rangeDb / 2;
	const yDb = (db: number) => a.y + a.h * (1 - clamp((db - bottomDb) / rangeDb, 0, 1.15));
	ctx.save();
	ctx.beginPath();
	ctx.rect(a.x, a.y, a.w, a.h);
	ctx.clip();
	// alpha base = fundido por silencio; los ajustes locales van relativos a él
	...
	// curva objetivo (referencia, offset 12 dB bajo el techo)
	...
	ctx.globalAlpha = A;
	if (f) {
		// relleno "Color Bars": curva suavizada con degradado del colormap por altura
		...
		ctx.closePath();
		ctx.fill();
		// línea FFT (con glow cálido suave, "lindo")
		...
		ctx.stroke();
		ctx.shadowBlur = 0;
		// lectura: cursor si está sobre el módulo, si no el pico dominante
		let rf = f[H.PEAK_HZ];
		let rdb = f[H.PEAK_DB] + tiltAt(Math.max(rf, 1), PRESET.spectrum.tilt);
		const m = S.mouse;
		const hover = !!m && m.x >= a.x && m.x < a.x + a.w && m.y >= a.y && m.y < a.y + a.h;
		let showReadout = hover || f[H.PEAK_DB] > -70; // en silencio no hay pico que leer
		if (hover && m) {
...
function drawStereometer(ctx: CanvasRenderingContext2D, a: Area, S: State, f: Float32Array | null, s: (px: number) => number) {
	const barW = s(6);
	const barBlock = s(34);
	const pad = s(6);
	...
	if (f) {
		// puntos por banda (Lissajous girado 45°: mono = vertical)
		...
		// normalize: el pico reciente llena el radio; ataque rápido, liberación lenta
		...
		// marcadores de correlación por banda + total
		const yc = (c: number) => by + ((1 - clamp(c, -1, 1)) / 2) * bh;
		const marks: [number, string][] = [
			[f[H.CORR_LOW], T.stLow],
			[f[H.CORR_MID], T.stMid],
			[f[H.CORR_HIGH], T.stHigh]
		];
		for (const [c, color] of marks) {
			ctx.fillStyle = color;
			ctx.fillRect(bx - s(1), yc(c) - s(1.5), barW + s(2), s(3));
		}
		ctx.fillStyle = T.stPrimary;
		ctx.fillRect(bx - s(2), yc(f[H.CORR]) - 0.5, barW + s(4), 1);
	}
	ctx.restore();
}

function drawLoudness(ctx: CanvasRenderingContext2D, a: Area, S: State, f: Float32Array | null, s: (px: number) => number) {
	const pad = s(9);
	const cx = a.x + a.w / 2;
	const st = f ? f[H.LUFS_S] : -120;
	const fmt = (v: number, d = 1) => (v <= SILENCE_LUFS ? "-inf" : v.toFixed(d));
	ctx.save();
	ctx.beginPath();
	ctx.rect(a.x, a.y, a.w, a.h);
	ctx.clip();
	// --- cabecera: número short-term grande ---
	...
	// barras: TP L, TP R (finas) + Short-term (ancha)
	...
	// línea de -14 LUFS (objetivo de streaming) en la barra ancha
	...
	ctx.stroke();
	if (f) {
		const bar = (x: number, w: number, v: number, hot: boolean) => {
			const y = yLu(v);
...
// ---- layout ----
// Jerarquía: los dos analizadores de frecuencia (spectrogram+spectrum) dominan la
// izquierda; medidores numéricos (loudness) y de fase (stereometer) en un raíl a la
// derecha; onda + osciloscopio en una tira inferior. Nada diminuto perdido en una esquina.
type Layout = {
	spectrogram: Area;
	spectrum: Area;
	waveform: Area;
...
function sliceCols(x: number, y: number, w: number, h: number, weights: number[], g: number): Area[] {
...
function layout(W: number, Hh: number, g: number): Layout {
	const aspect = W / Hh;
	// Barra muy ancha (visualizer acoplado tipo barra): una sola fila, como la app.
	if (aspect > 2.4) {
		const r = sliceCols(0, 0, W, Hh, [3, 3, 2, 1.3, 1.7, 1.6], g);
		return {
			spectrogram: r[0], spectrum: r[1], waveform: r[2],
			oscilloscope: r[3], stereometer: r[4], loudness: r[5]
		};
	}
	// Parrilla para área grande (~4:3): raíl derecho + tres filas a la izquierda.
	const RW = Math.round(W * (aspect < 1.05 ? 0.3 : 0.26));
	const railX = W - RW;
	const MW = railX - g;
	// raíl derecho: Loudness arriba (números que se miran), Stereometer cuadrado abajo.
	const sq = Math.min(RW, Math.round(Hh * 0.44));
	const loudness: Area = { x: railX, y: 0, w: RW, h: Hh - sq - g };
	const stereometer: Area = { x: railX, y: Hh - sq, w: RW, h: sq };
	// izquierda: Spectrum (el principal) el más alto, Spectrogram encima, tira inferior.
	const h1 = Math.round(Hh * 0.36); // spectrogram
	const h2 = Math.round(Hh * 0.4); // spectrum
	const h3 = Hh - h1 - h2 - g * 2; // tira: waveform + oscilloscope
	const spectrogram: Area = { x: 0, y: 0, w: MW, h: h1 };
	const spectrum: Area = { x: 0, y: h1 + g, w: MW, h: h2 };
...
export default function MiniMetersVisualizer(props: RendererProps) {
	const stateRef = useRef<State | null>(null);

	// Cambio de pista -> reiniciar el LUFS integrado del bridge
	useEffect(() => {
		stateRef.current?.link?.send("reset");
	}, [props.songToken]);
	...
	useEffect(
		() => () => {
			...
			if (!S) return;
			S.link?.close();
			if (S.canvas && S.onMove) S.canvas.removeEventListener("mousemove", S.onMove);
			if (S.canvas && S.onLeave) S.canvas.removeEventListener("mouseleave", S.onLeave);
		},
		[]
	);

	const data = useMemo(() => ({ token: props.songToken }), [props.songToken]);

	const onInit = useCallback((ctx: CanvasRenderingContext2D | null): State => {
		const S = makeState();
		if (!ctx) {
			console.error("MiniMeters: canvas 2D no soportado");
			S.isError = true;
			stateRef.current = S;
			return S;
		}
		const win = ctx.canvas.ownerDocument.defaultView as Window;
		S.link = new MeterLink(win);
		S.canvas = ctx.canvas;
		S.onMove = (e: MouseEvent) => {
		...
		ctx.canvas.addEventListener("mousemove", S.onMove);
		ctx.canvas.addEventListener("mouseleave", S.onLeave);
		stateRef.current = S;
		return S;
	}, []);
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, _data: unknown, S: State) => {
		if (!ctx || S.isError) return;
		try {
			const now = ctx.canvas.ownerDocument.defaultView?.performance.now() ?? 0;
			// cap del render a 144 fps (acumulador): con --disable-frame-rate-limit el rAF
			// sube a ~200; esto evita dibujar por encima de lo que el monitor 144 Hz muestra.
			if (now < S.nextRenderT) return;
			S.nextRenderT = Math.max(now, S.nextRenderT + 1000 / 144);
			const W = ctx.canvas.width;
			const Hh = ctx.canvas.height;
			const dpr = ctx.canvas.ownerDocument.defaultView?.devicePixelRatio ?? 1;
			const s = (px: number) => px * dpr;
			ctx.fillStyle = T.bg;
			...
			// nº de frames de datos nuevos desde el último dibujo: deja que el
			// espectrograma scrollee a la tasa de DATOS (144) aunque Spotify cape el render a 60.
			...
			// fade_modules_during_silence: atenuar tras 1 s de silencio
			...
			ctx.globalAlpha = 1;
			// bordes finos entre módulos
			ctx.strokeStyle = T.bg2;
			ctx.lineWidth = 1;
			for (const a of all)
				ctx.strokeRect(Math.round(a.x) + 0.5, Math.round(a.y) + 0.5, Math.round(a.w) - 1, Math.round(a.h) - 1);
			...
			if (!live) {
				...
				ctx.textAlign = "center";
				ctx.textBaseline = "middle";
				ctx.fillText(`bridge sin conexión (${BRIDGE_URL}) — arranca minimeters-bridge`, W / 2, Hh / 2);
			}
		} catch {
			// no dejar que un error puntual de dibujo tumbe la ruta del visualizer
		}
	}, []);
